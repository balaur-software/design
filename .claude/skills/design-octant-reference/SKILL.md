...
`@balaur/octant` (v0.3.0 as of 2026-07-07). The three `packages/*` workspaces
...
As of 2026-07-07, HEAD is `74aad33` ("Storybook 10 + Storybook Test, SSR
gates, and full library audit fixes"), **two** commits past tag `v0.3.0`, and
`bun run check` **PASSES** (241 tests / 17 files, exit 0; one non-blocking
biome "migrate configuration" info). The previously documented red HEAD
(`7073f40`, 2 TS errors in story files) was fixed by `74aad33` — if you read
older notes calling design HEAD red, they are stale. Gate policy lives in
design-change-and-release.
...
`packages/tokens/src/tokens.css` (89 `--bx-*` declarations as of 2026-07-07).
...
**111 components: 41 atoms / 39 molecules / 31 organisms** (counted 2026-07-07
at `74aad33`), plus 3 primitives (`FloatingPanel`, `ScrimOverlay`,
...
5. **Imports are RELATIVE, with extensions.** `allowImportingTsExtensions` is
   ...
   **Never** import `@balaur/octant-core` / `@balaur/tokens` / `@balaur/ui`
   ...
   Verified 2026-07-07: zero `from "@balaur/` occurrences in `packages/ui/src`.
...
| `ssr-stories.test.tsx` | EVERY story of EVERY component (`**/*.stories.tsx`, 119 files as of 2026-07-07) server-renders via portable stories. Empty output is allowed (closed overlays); throwing is not |
...
2. **Built but unconsumed** as of 2026-07-07 — nothing in `web/` imports them
   yet; they exist ahead of the campaign.
...
install; the `74aad33` commit message reports 464 story tests green, but this
was **not re-run while authoring this skill** (heavy browser run) — treat
current green-ness as unverified until you run it. The plain-`bun test` SSR
story gate (above) is the cheap always-on proxy.
...
`v0.3.0`). Drift-prone facts and their one-line re-checks (run from
`/home/alex/projects/balaur/design`):
...
| `bun run check` PASSES; 241 tests / 17 files | `bun run check` |
